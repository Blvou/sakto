...
import { CategoryBreadcrumb } from '@/src/features/listings/components/CategoryBreadcrumb';
import {
...
export default function BrowseCategoryScreen() {
  ...
  useEffect(() => {
    if (slug === 'marketplace') {
      router.replace('/marketplace/search' as Href);
    }
  }, [router, slug]);

  const isValidSlug = isListingBrowseSlug(slug);
  const dbCategory = slug;
    ...
    return (
      <View style={{ flex: 1, backgroundColor: colors.background }}>
        ...
        <EmptyState
          ...
          onAction={() => router.push('/marketplace/categories' as Href)}
        />
      </View>
    );
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <CategoryBreadcrumb categoryId={slug} />
